...
const OurGoal = () => {
	document.title = "Site Setting: Our Goal | Admin & Dashboard";
	...
	useEffect(() => {
		setFile([]);
		dispatch(getSiteSetting("OUR_GOAL"));
		return () => {
	...
	const settingForm = useFormik({
		...
		initialValues: {
			...
			vision: siteSetting ? siteSetting.vision : "",
			mission: siteSetting ? siteSetting.mission : "",
			ourvalue: siteSetting ? siteSetting.ourvalue : "",
	...
	useEffect(() => {
		if (siteSetting) {
			...
			if (siteSetting.thumbnail) {
				setFileTwo([
					{
						source: siteSetting.thumbnailTwo,
						options: {
							type: "local",
						},
					},
									...
									<CardBody>
										{isLoading ? (
											...
											<Row>
												<Col xl={12}>
													<div className="mb-3">
														<Label className="form-label" htmlFor="vision-input">
															Vision
														</Label>
														<textarea
															className="form-control"
															...
															onChange={settingForm.handleChange}
															onBlur={settingForm.handleBlur}
															value={settingForm.values.vision}
														></textarea>
													</div>
													<div className="mb-3">
														<Label className="form-label" htmlFor="mission-input">
															Mission
														</Label>
														<textarea
															className="form-control"
															...
															onChange={settingForm.handleChange}
															onBlur={settingForm.handleBlur}
															value={settingForm.values.mission}
														></textarea>
													</div>
													<div className="mb-3">
														<Label className="form-label" htmlFor="ourvalue-input">
															Our Value
														</Label>
														<textarea
															className="form-control"
															...
															onChange={settingForm.handleChange}
															onBlur={settingForm.handleBlur}
															value={settingForm.values.ourvalue}
														></textarea>
													</div>
												</Col>
											</Row>
...
export default withRouter(OurGoal);
