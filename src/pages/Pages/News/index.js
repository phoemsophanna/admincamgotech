...
import { deleteNews, fetchNewsList, refreshNewsList } from "../../../store/actions";
...
const NewsMenu = () => {
	...
	const newsListSelector = createSelector(
		...
		(layout) => ({
			newsList: layout.newsList,
			isLoading: layout.isLoading,
		})
	);
	const { newsList, isLoading } = useSelector(newsListSelector);
	const deleteNewsSelector = createSelector(
		(state) => state.NewsListReducer,
		(layout) => layout
	);
	const newsSelector = useSelector(deleteNewsSelector);
	...
	const columns = useMemo(
		() => [
						...
						<div className="d-flex align-items-center">
							<div className="flex-shrink-0 me-3">
								<div className="avatar-sm bg-light rounded p-1 d-flex align-items-center">
									{news.row.original.image ? (
										<img src={api.FILE_URI + news.row.original.image} alt="" className="img-fluid d-block" />
									) : (
							...
							<div className="flex-grow-1">
								<h5 className="fs-14 mb-1">
									<Link to="#" className="text-body">
										{news.row.original.title}
									</Link>
								</h5>
								<p className="text-muted text-truncate mb-0" style={{ maxWidth: 200 }}>
									<span className="fw-medium">{news.row.original.summary}</span>
								</p>
							</div>
						</div>
					</>
			...
			},
			{
				...
				Cell: (news) => (
					<>
						{news.row.original.isDisplayHomepage ? (
					...
					return (
						<ul className="list-inline hstack gap-2 mb-0">
							<li className="list-inline-item" title="Edit">
								<Link className="edit-item-btn" to={`/news-menu/edit/${cellProps.row.original.id}`}>
	...
	return (
		<React.Fragment>
							...
							<Card>
								<CardHeader>
									<Row className="justify-content-between align-items-center gy-3">
										<Col lg={3}>
											<Link className="btn add-btn btn-primary" to="/news-menu/create">
			...
			<DeleteModal show={deleteModal} onDeleteClick={handleDelete} onCloseClick={() => setDeleteModal(false)} isLoading={newsSelector.isLoading} />
		</React.Fragment>
	);
};
