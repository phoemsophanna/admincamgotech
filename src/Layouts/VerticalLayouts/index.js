...
const VerticalLayout = (props) => {
    ...
    */
	...
	const { layoutType, leftsidbarSizeType, sidebarVisibilitytype } = useSelector(selectLayoutProperties);
	...
	useEffect(() => {
		...
		const initMenu = () => {
			...
			let matchingMenuItem = itemsArray.find((x) => {
				return pathName.search(x.pathname) >= 0 || x.pathname === pathName;
			});
			if (matchingMenuItem) {
																							...
																							<ul className="nav nav-sm flex-column">
																								{childItem.childItems.map((subChildItem, key) => (
																									<li className="nav-item" key={key}>
																										<Link to={subChildItem.link} className="nav-link" data-key="t-basic-action">
